...
import { DataStack, APIStack } from "../../../deploy-output.json";
import { SpaceEntry } from "../components/model/model";

const SPACES_URL = `${APIStack.SpacesApiEndpoint36C4F3B6}spaces`;
...
export class DataService {
  ...
  public async getSpaces(): Promise<SpaceEntry[]> {
    const getSpacesResult = await fetch(SPACES_URL, {
      method: "GET",
      headers: {
  ...
  public async createSpace(name: string, location: string, photo?: File) {
    ...
    const postResult = await fetch(SPACES_URL, {
      method: "POST",
